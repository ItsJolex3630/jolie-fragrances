...
export const RETAIL_PRICES: Record<number, number | null> = {
  ...
  225: 34,
  226: 40,  // Khair Fusion (catálogo $30 → +35% = $41)
  227: 40,  // Khair Confection (catálogo $30 → +35% = $41)
  232: 55,  // Indomitable (catálogo $41 → +35% = $55)
  233: 72,  // Marshmallow Blush (catálogo $53 → +35% = $72)
  234: 32,  // Khair (catálogo $24 → +35% = $32)
  235: 43,  // Khair Felicity (catálogo $33 → +35% = $45)
  236: 43,  // December (catálogo $42 → +35% = $57)
  237: 57,  // Emir Memories of Summer (catálogo $42 → +35% = $57)
  238: 54,  // Taskeen (catálogo $40 → +35% = $54)
  239: 40,  // Taskeen Marina (catálogo $30 → +35% = $41)
  ...
  244: 40,  // Pear Potion (catálogo $32 → +35% = $43)
...
/**
 * Round a price using "round half up" rule (0.5 rounds up, below 0.5 rounds down).
 * Examples: 54.5 → 55, 54.3 → 54, 54.7 → 55, 48.6 → 49.
 *
 * Uses a small epsilon to handle floating-point edge cases (e.g., 54.5 stored
 * as 54.49999999 in IEEE 754).
 */
export function roundPrice(price: number): number {
  return Math.round(price + 1e-9);
}

/**
 * Apply a discount percentage to a price and round the result.
 * Returns the discounted + rounded price.
 *
 * Example: applyDiscount(54, 10) → round(54 * 0.9) = round(48.6) = 49
 */
...
/**
 * Format a price with optional discount for display.
 * - No discount: "$54"
 * - With discount: shows original with strikethrough + discounted (caller handles HTML)
 *   This function just returns the discounted number; the UI handles the visual.
 */
export function formatDiscountedPrice(
  ...
  return {
    ...
    discounted: applyDiscount(originalPrice, discountPct),
    hasDiscount: true,
  };
}