...
export default function SimilarPerfumesModal({
  ...
  useEffect(() => {
    ...
    if (similarParam && similarParam !== "true") {
      const id = similarParam.toLowerCase();
      const found = allPerfumes.find(
        (p) => (p.perfumeSlug || "").toLowerCase() === id || p.id.toString() === id
      );
      if (found) {
  ...
  const updateUrl = (p: Perfume | null) => {
    if (typeof window === "undefined") return;
    const url = new URL(window.location.href);
    if (p) {
      url.searchParams.set("similar", p.perfumeSlug || p.id.toString());
    } else {
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-50 bg-[#0a0a0a]/98 overflow-y-auto"
        >
          <div className="min-h-full flex flex-col">
            {/* Header */}
            <div className="sticky top-0 z-10 bg-[#0a0a0a]/97 border-b border-[rgba(212,175,55,0.08)]">
