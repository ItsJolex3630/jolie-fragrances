    ...
    >
      <div className="relative overflow-hidden rounded-xl border border-[rgba(212,175,55,0.12)] bg-[#111111] transition-all duration-500 group-hover:border-[rgba(212,175,55,0.35)] gold-glow-hover">
        ...
        <div className="relative aspect-[3/4] overflow-hidden bg-[#0a0a0a] flex items-center justify-center">
          ...
          {/* Heart/Favorite button - OUTSIDE anchor to prevent click conflicts */}
          {isLoggedIn && (
            <button
              onClick={(e) => {
                e.preventDefault();
              ...
              className="absolute top-2 left-2 z-30 p-1.5 rounded-full transition-all duration-200 cursor-pointer hover:scale-110 active:scale-90"
              style={{
...
export default function Home() {
  ...
  // ─── Session check & perfume fetch on mount ───
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        // Fallback: empty list, already set
      }
  ...
  // ─── Ref for stale-closure-safe favorite toggle ───
  const favoritesRef = useRef(favorites);
  useEffect(() => { favoritesRef.current = favorites; }, [favorites]);

  // ─── Toggle favorite ───
  const handleToggleFavorite = useCallback(async (perfumeId: number) => {
    const isFav = favoritesRef.current.includes(perfumeId);
    // Optimistic update
    setFavorites((prev) =>
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({
          ...
          description: isFav
            ? "La fragancia fue removida de tus favoritos"
            : "La fragancia fue guardada en tus favoritos",
        });
      } else {
        // Revert on error
        setFavorites((prev) =>
          isFav ? [...prev, perfumeId] : prev.filter((id) => id !== perfumeId)
        );
        toast({
          title: "Error",
  ...
  // ─── Remove favorite (from FavoritesView) ───
  const handleRemoveFavorite = useCallback(async (perfumeId: number) => {
    try {
      const res = await fetch("/api/favorites", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        toast({
          ...
          description: "La fragancia fue removida de tus favoritos",
        });
      }
    } catch {
      // Silently fail
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32 text-center">
          ...
          <div className="absolute top-4 right-4 z-20">
            {user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="flex items-center gap-2 px-3 py-2 rounded-full border border-[rgba(212,175,55,0.25)] bg-[rgba(17,17,17,0.8)] backdrop-blur-sm text-[#d4af37] hover:bg-[rgba(212,175,55,0.1)] transition-all duration-300 font-[family-name:var(--font-inter)] text-xs sm:text-sm"
                  >
                ...
                <DropdownMenuContent
                  align="end"
                  className="bg-[#111111] border-[rgba(212,175,55,0.2)] text-white w-48"
                >
                  <div className="px-2 py-1.5">
                    <p className="text-xs text-white/40 font-[family-name:var(--font-inter)] truncate">
                      {user.email}
                    </p>
                  </div>
                  ...
                  <DropdownMenuItem
                    ...
                    className="text-white/70 hover:text-white hover:bg-[rgba(212,175,55,0.1)] focus:bg-[rgba(212,175,55,0.1)] cursor-pointer font-[family-name:var(--font-inter)] text-xs"
                  >
                    <Heart className="w-3.5 h-3.5 text-red-400" />
                    Mis Favoritos
                    {favorites.length > 0 && (
                      <span className="ml-auto bg-[#d4af37]/15 text-[#d4af37] text-[9px] px-1.5 py-0.5 rounded-full">
                        {favorites.length}
                      </span>
                    )}
                  </DropdownMenuItem>
                  ...
                  <DropdownMenuItem
                    ...
                    className="text-red-400/70 hover:text-red-400 hover:bg-red-400/10 focus:bg-red-400/10 cursor-pointer font-[family-name:var(--font-inter)] text-xs"
                  >
              ...
              <motion.button
                ...
                className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full border border-[rgba(212,175,55,0.25)] bg-[rgba(17,17,17,0.8)] backdrop-blur-sm text-[#d4af37] hover:bg-[rgba(212,175,55,0.1)] transition-all duration-300 font-[family-name:var(--font-inter)] text-xs sm:text-sm"
              >
                <User className="w-3.5 h-3.5" />
                Iniciar Sesión
              </motion.button>
          ...
          >
            <motion.button
              ...
              className="px-6 py-3 rounded-full bg-gradient-to-r from-[#d4af37] to-[#b8941e] text-[#0a0a0a] font-semibold text-sm font-[family-name:var(--font-inter)] shadow-lg shadow-[#d4af37]/20"
            >
              Explorar Catálogo
            </motion.button>
            <a
          ...
          >
            {paginatedPerfumes.map((perfume, index) => (
              <PerfumeCard
                ...
                perfume={perfume}
                index={index}
                isFavorited={favorites.includes(perfume.id)}
                isLoggedIn={!!user}
      ...
      <AnimatePresence>
        {showScrollTop && (
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            onClick={scrollToTop}
            className="fixed bottom-6 left-6 z-50 bg-[#d4af37] text-[#0a0a0a] rounded-full p-3 shadow-lg shadow-[#d4af37]/20 hover:shadow-[#d4af37]/30 transition-shadow duration-300"
      ...
      <AnimatePresence>
        {selectedPerfume && (
          <PerfumeDetail
            ...
            isFavorited={favorites.includes(selectedPerfume.id)}
            isLoggedIn={!!user}
